...
similarLoinc  <- loinc_de %>%
  ...
  mutate(SIMILAR_PROP = if_else(is.na(PROPERTY_GROUP), 
                                 PROPERTY,
                                 PROPERTY_GROUP)) %>%
  group_by(COMPONENT, TIME_ASPCT, SYSTEM, SCALE_TYP, SIMILAR_PROP) %>%
  ...
  select(GROUP_UID, LONG_COMMON_NAME, LOINC_NUM, COMPONENT, TIME_ASPCT, 
         SYSTEM, SCALE_TYP, PROPERTY, SIMILAR_PROP, CLASS)
